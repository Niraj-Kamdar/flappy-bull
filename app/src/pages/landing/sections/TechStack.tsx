...
export function TechStack() {
  return (
    ...
    >
      <ScrollReveal>
        <div className="glass-card overflow-hidden mb-8">
          {stack.map(({ layer, tech }) => (
            <div
              key={layer}
              className="grid grid-cols-2 border-b border-neon-purple/20 last:border-0 hover:bg-neon-purple/10 transition-colors"
            >
              <div className="p-5 font-mono text-xs text-neon-amber tracking-widest border-r border-neon-purple/20">
                {layer}
              </div>
              <div className="p-5 font-mono text-sm text-text-primary">{tech}</div>
            </div>
          ))}
        </div>
      </ScrollReveal>
      ...
      <ScrollReveal>
        <div className="border border-neon-teal/30 rounded-xl p-6 bg-neon-teal/5 text-center card-hover">
